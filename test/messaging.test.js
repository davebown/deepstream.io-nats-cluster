...
const getNode = async (serverName, servers) => {
...
describe('Messages are sent between multiple instances', () => {
  ...
  before(async () => {
    nodeA = await getNode('node-A', NATS_SERVERS);
    nodeB = await getNode('node-B', NATS_SERVERS);
    nodeC = await getNode('node-C', NATS_SERVERS);
  });
